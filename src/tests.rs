...
use crate::{TurboShake128, TurboShake256, TurboShakeError};
use rand::prelude::*;
use std::cmp;
...
fn turboshake128<const D: u8>(msg: &[u8]) -> TurboShake128 {
    ...
    assert_eq!(hasher.absorb(msg), Ok(()));
    assert_eq!(hasher.finalize::<D>(), Ok(()));
...
fn test_turboshake128() {
    let mut out = [0u8; 32];
    assert_eq!(turboshake128::<0x01>(&[0u8; 0]).squeeze(&mut out), Ok(()));
...
fn test_turboshake256() {
    let mut out = [0u8; 32];
    assert_eq!(turboshake256::<0x01>(&[0u8; 0]).squeeze(&mut out), Ok(()));
...
fn test_incremental_ts128_hashing(mlen: usize, dlen: usize) {
    ...
    assert_eq!(hasher_oneshot.absorb(&msg), Ok(()));
    assert_eq!(hasher_oneshot.finalize::<{ TurboShake128::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher_oneshot.squeeze(&mut md_oneshot), Ok(()));
    ...
    while msg_offset < mlen {
        ...
        let elen = (msg[msg_offset].max(1) as usize).min(mlen - msg_offset);

        assert_eq!(hasher_incremental.absorb(&msg[msg_offset..(msg_offset + elen)]), Ok(()));
        msg_offset += elen;
    }

    assert_eq!(hasher_incremental.finalize::<{ TurboShake128::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));

    let mut md_offset = 0;
    while md_offset < dlen {
        assert_eq!(hasher_incremental.squeeze(&mut md_incremental[md_offset..(md_offset + 1)]), Ok(()));
        md_offset += 1;

        let elen = (md_incremental[md_offset - 1] as usize).min(dlen - md_offset);
        assert_eq!(hasher_incremental.squeeze(&mut md_incremental[md_offset..(md_offset + elen)]), Ok(()));
...
fn test_incremental_ts256_hashing(mlen: usize, dlen: usize) {
    ...
    assert_eq!(hasher_oneshot.absorb(&msg), Ok(()));
    assert_eq!(hasher_oneshot.finalize::<{ TurboShake256::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher_oneshot.squeeze(&mut md_oneshot), Ok(()));
    ...
    assert_eq!(hasher_incremental.finalize::<{ TurboShake256::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
...
fn state_transition_should_work_in_ts128() {
    ...
    let mut hasher = TurboShake128::default();
    assert_eq!(hasher.absorb(msg), Ok(()));
    assert_eq!(hasher.finalize::<{ TurboShake128::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher.squeeze(&mut md), Ok(()));
    ...
    assert_eq!(
        ...
        Err(TurboShakeError::DataAbsorptionPhaseAlreadyFinalized)
    );
    assert_eq!(hasher.squeeze(&mut md), Ok(()));

    // squeeze -> finalize -> squeeze
    let mut hasher = TurboShake128::default();
    assert_eq!(hasher.squeeze(&mut md), Err(TurboShakeError::StillInDataAbsorptionPhase));
    assert_eq!(hasher.finalize::<{ TurboShake128::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher.squeeze(&mut md), Ok(()));
    ...
    assert_eq!(hasher.absorb(msg), Ok(()));
    assert_eq!(hasher.finalize::<{ TurboShake128::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher.absorb(msg), Err(TurboShakeError::DataAbsorptionPhaseAlreadyFinalized));
...
fn state_transition_should_work_in_ts256() {
    ...
    let mut hasher = TurboShake256::default();
    assert_eq!(hasher.absorb(msg), Ok(()));
    assert_eq!(hasher.finalize::<{ TurboShake256::DEFAULT_DOMAIN_SEPARATOR }>(), Ok(()));
    assert_eq!(hasher.squeeze(&mut md), Ok(()));
